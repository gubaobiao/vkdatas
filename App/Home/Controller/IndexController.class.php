    ...
    public function _initialize()
    {
        ...
        foreach ($typeIds as $key=>$value) {
            ...
            for ($i=0;$i<$count;$i++){
                ...
                $extStamp = $nowStamp - $pubtime;
                if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                    $anli[$value['id']][$i]['pubday'] = floor($extStamp/3600).'小时前';
                }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                    $anli[$value['id']][$i]['pubday'] = floor($extStamp/24/3600).'天前';
                }elseif ($extStamp > 24*3600*7){
                    $anli[$value['id']][$i]['pubday'] = date('Y/m/d',$pubtime);
                }elseif ($extStamp < 3600){
                    $anli[$value['id']][$i]['pubday'] = floor($extStamp/60).'分钟前';
                }
            }
        }
...
//            $news[$key]['pubday']=floor((time()-$pubtime)/24/60/60);
            $news[$key]['timer'] = $news[$key]['timer']== 0 ? strtotime($news[$key]['addtime']):$news[$key]['timer'];
            $nowStamp = time();
            $extStamp = $nowStamp - $news[$key]['timer'];
            if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                $news[$key]['pubday'] = floor($extStamp/3600).'小时前';
            }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                $news[$key]['pubday'] = floor($extStamp/24/3600).'天前';
            }elseif ($extStamp > 24*3600*7){
                $news[$key]['pubday'] = date('Y/m/d',$news[$key]['timer']);
            }elseif ($extStamp < 3600){
                $news[$key]['pubday'] = floor($extStamp/60).'分钟前';
            }
        }
    ...
    public function news()
    {
        ...
        if(empty($id)) {
            ...
            $news=M('news')->where('is_show=1 and timer <='.time())->order('timer desc')->limit(5)->select();
            //数据处理
            foreach ($news as $key => $v) {
                $news[$key]['content']=mb_substr(strip_tags(str_replace('&nbsp;','',$v['content'])),0,200,'utf-8').'......';
...
//            $info['pubday']=floor((time()-$info['addtime'])/24/60/60);
            $info['share_url']= $_SERVER['HTTP_HOST'].U("/news/$id");
            ...
            if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                $info['pubday'] = floor($extStamp/3600).'小时前';
            }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                $info['pubday'] = floor($extStamp/24/3600).'天前';
            }elseif ($extStamp > 24*3600*7){
                $info['pubday'] = date('Y年m月d日',$info['timer']);
            }elseif ($extStamp < 3600){
                $info['pubday'] = floor($extStamp/60).'分钟前';
            }

            //标签属性过滤
            $pattern = '/<([a-z]*?)\s+?.*?>/i';
            //file_put_contents('D:\ftp\wap_description_before.txt',$info['content']);
            $info['description'] = str_replace(PHP_EOL, '', $info['description']);
            //file_put_contents('D:\ftp\wap_description_before.txt',$info['content']);
            $pattern = '/<([^ai][a-z]*?)\s+?.*?>/i';
            $info['content'] = preg_replace($pattern,'<$1>',$info['content']);
//            $pattern = '/<img\s.*(src="[^\s]*")\s.*(title="[^\s]*")\s.*(alt=".*")\s.*(width=".*")\s.*(height=".*")\s.*(style=".*")\/>/i';
//            $info['content'] = preg_replace($pattern,'<img $1 $2 $3 />',$info['content']);

            $pattern = '/<img [^>]*(src=\"[^\s]*\")[^>]*(title=\"[^\s]*\")[^>]*(alt=\"(.+?)\")[^>]*\/>/i';
            $info['content'] = preg_replace($pattern,'<img  $1 $2 $3 />',$info['content']);

            //file_put_contents('D:\ftp\wap_news_after.txt',$info['content']);
            $this->assign('info',$info);

            //关键词优化
            $seoWord = [];
            $seoWord = $this->setSeoWord($seoWord,3);
            $seoWord['title']       = $info['title'].'-动画街';
    ...
    public function worker()
    {
        ...
        $i=0;
        foreach ($typeIds as $v=>$key){
            $where['type'] = $key['id'];
            $typeIds[$i]['count'] = M('video')->where($where)->count();
            $i++;
        }
    ...
    public function anli()
    {
        ...
        if(empty($id)) {
            ...
            //查询
            $video=M('video')->field(['id','createtime','timing','title','type','sltpath'])->where($where)->limit(5)->order('id desc')->select();
...
//                $video[$key]['pubday']=floor((time()-$pubtime)/24/60/60);
                ...
                $extStamp = $nowStamp - $pubtime;
                if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                    $video[$key]['pubday'] = floor($extStamp/3600).'小时前';
                }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                    $video[$key]['pubday'] = floor($extStamp/24/3600).'天前';
                }elseif ($extStamp > 24*3600*7){
                    $video[$key]['pubday'] = date('Y/m/d',$pubtime);
                }elseif ($extStamp < 3600){
                    $video[$key]['pubday'] = floor($extStamp/60).'分钟前';
                }
            }
...
//                $videoData['pubday']=floor((time()-$pubtime)/24/60/60);
                ...
                //判断时间
                $videoData['timing'] = $videoData['timing']== 0 ? strtotime($videoData['create_time']):$videoData['timing'];
                $nowStamp = time();
                $extStamp = $nowStamp - $videoData['timing'];
                if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                    $videoData['pubday'] = floor($extStamp/3600).'小时前';
                }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                    $videoData['pubday'] = floor($extStamp/24/3600).'天前';
                }elseif ($extStamp > 24*3600*7){
                    $videoData['pubday'] = date('Y年m月d日',$videoData['timing']);
                }elseif ($extStamp < 3600){
                    $videoData['pubday'] = floor($extStamp/60).'分钟前';
                }

                //除去标签属性  除了img和a标签

                //file_put_contents('D:\ftp\wap_description_before.txt',$videoData['description']);
                $videoData['description'] = str_replace(PHP_EOL, '', $videoData['description']);
                //file_put_contents('D:\ftp\wap_description_before.txt',$videoData['description']);
                //file_put_contents('D:\ftp\wap_video_before.txt',$videoData['content']);
                $pattern = '/<([^ai][a-z]*?)\s+?.*?>/i';
                $videoData['content'] = preg_replace($pattern,'<$1>',$videoData['content']);
//                $pattern = '/<img\s(onload=.*")\s(src="[^\s]*")\s(title="[^\s]*")\s(alt=".*")>/i';
//                $videoData['content'] = preg_replace($pattern,'<img $1 $2 $3 />',$videoData['content']);

                $pattern = '/<img [^>]*(src=\"[^\s]*\")[^>]*(title=\"[^\s]*\")[^>]*(alt=\"(.+?)\")[^>]*\/>/i';
                $videoData['content'] = preg_replace($pattern,'<img  $1 $2 $3 />',$videoData['content']);

                //file_put_contents('D:\ftp\wap_video_after.txt',$videoData['content']);
                ...
                $seoWord['title']       = $videoData['title'].'-动画街';
                //随机5关键词
                $randArr = array_rand($seoWord['keywords'],5);
                $tmpWord = [];
                foreach ($randArr as $key=>$value){
            ...
            foreach ($video_all as $key=>$value){
                $typeNmae = M('video_type')->where(['id'=>$video_all[$key]['type']])->find();
                $video_all[$key]['typeTitle'] = $typeNmae['type_name'];

                $nowStamp = time();
                $pubtime= $video_all[$key]['timing']!=0?$video_all[$key]['timing']: strtotime($video_all[$key]['createtime']);
                $extStamp = $nowStamp - $pubtime;
                if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                    $video_all[$key]['pubday'] = floor($extStamp/3600).'小时前';
                }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                    $video_all[$key]['pubday'] = floor($extStamp/24/3600).'天前';
                }elseif ($extStamp > 24*3600*7){
                    $video_all[$key]['pubday'] = date('Y/m/d',$pubtime);
                }elseif ($extStamp < 3600){
                    $video_all[$key]['pubday'] = floor($extStamp/60).'分钟前';
                }
            }
    ...
    public function videolazy(){
        ...
        //查询
        $countNum = M('video')->where($where)->count();
        $videoData = M('video')->where($where)->page($pageNum,$pages)->order(['id'=>'desc'])->select();
...
//                $videoData[$key]['pubday']=floor((time()-$pubtime)/24/60/60);
                ...
                $extStamp = $nowStamp - $pubtime;
                if ($extStamp <= 24*3600 && $extStamp >= 3600) {
                    $videoData[$key]['pubday'] = floor($extStamp/3600).'小时前';
                }elseif ($extStamp > 24*3600 && $extStamp <= 24*3600*7){
                    $videoData[$key]['pubday'] = floor($extStamp/24/3600).'天前';
                }elseif ($extStamp > 24*3600*7){
                    $videoData[$key]['pubday'] = date('Y/m/d',$pubtime);
                }elseif ($extStamp < 3600){
                    $videoData[$key]['pubday'] = floor($extStamp/60).'分钟前';
                }
            }
            $data['data'] = $videoData;
    ...
    //动态下拉加载接口
    public function newslazy(){

        $pageNum = I('get.page_num');
        $pages = I('get.pages',5);

        //参数判断
        if(!is_numeric($pageNum) || !is_numeric($pages)){
            $data['msg'] = '非法参数';
            $data['status'] = 10000;
            $data['data'] = [];
            $data['total'] = [];
        ...
        //构建查询语句
        $where['timer'] = ['elt',time()];
        $where['is_show'] = 1;

        //查询
        $countNum =M('news')->where($where)->count();
        $news=M('news')->where($where)->order('timer desc')->page($pageNum,$pages)->select();
